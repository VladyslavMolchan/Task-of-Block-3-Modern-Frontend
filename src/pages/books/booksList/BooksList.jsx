...
const BooksList = () => {
  ...
  const [showFilters, setShowFilters] = useState(() =>
    !!(
      ...
      searchParams.get("yearPublished")
    )
  );
  ...
  useEffect(() => {
    const loadBooks = async () => {
      ...
      try {
        const hasFilters =
          filters.title || filters.authorId || filters.yearPublished;

        const data = hasFilters
          ? await searchBooks(
              {
                title: filters.title.trim() || undefined,
                authorId: !isNaN(filters.authorId)
                  ? Number(filters.authorId)
                  : undefined,
                yearPublished: !isNaN(filters.yearPublished)
                  ? Number(filters.yearPublished)
                  : undefined,
              },
              page,
              size
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const updatedPage = page > 1 && books.length === 1 ? page - 1 : page;
      changePage({ locationSearch: { ...filters, page: updatedPage } });
    } catch {
      setNotification({
        open: true,
        type: "error",
  ...
  return (
    <Paper sx={{ p: 3}}>
      ...
      <BooksTable
        ...
        loading={loading}
        onDeleteClick={(id) =>
          setDeleteDialog({ open: true, bookId: id, error: "" })
        }
        onRowClick={(id) =>
      ...
      <Pagination
        ...
        onChange={(p) =>
          changePage({ locationSearch: { ...filters, page: p } })
        }
        sx={{ mb: 2 }}
      />
      ...
      <DeleteDialog
        ...
        onConfirm={() => handleDelete(deleteDialog.bookId)}
